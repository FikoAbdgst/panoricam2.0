...
@section('hero_section')
    <style>
        ...
        .frame-container {
            position: relative;
            min-height: 200px;
            /* Ensure container has minimum height for loading */
        }
    </style>
    <div class="py-16 bg-[#FEF3E2] content_section pt-32">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 backdrop-blur-sm bg-white/70 rounded-2xl shadow-lg border border-gray-100 p-6">
                <div class="flex flex-col sm:flex-row gap-6 justify-between items-center">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-gray-700">Filter Rating:</span>
                        </div>

                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('frametemp', array_merge(request()->all(), ['sort_rating' => 'desc'])) }}"
                                class="rating-filter-btn group relative px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 border-2
                                      {{ request('sort_rating') === 'desc' ? 'bg-gradient-to-r from-[#BF3131] to-red-600 text-white border-[#BF3131] shadow-lg transform scale-105' : 'bg-white text-gray-600 border-gray-200 hover:border-[#BF3131] hover:text-[#BF3131] hover:bg-red-50' }}">
                                <div class="flex items-center gap-2">
                            ...
                            <a href="{{ route('frametemp', array_merge(request()->all(), ['sort_rating' => 'asc'])) }}"
                                class="rating-filter-btn group relative px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 border-2
                                      {{ request('sort_rating') === 'asc' ? 'bg-gradient-to-r from-[#BF3131] to-red-600 text-white border-[#BF3131] shadow-lg transform scale-105' : 'bg-white text-gray-600 border-gray-200 hover:border-[#BF3131] hover:text-[#BF3131] hover:bg-red-50' }}">
                                <div class="flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <a href="{{ route('frametemp', request()->except('sort_rating')) }}"
                                class="rating-filter-btn group relative px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 border-2
                                      {{ !request('sort_rating') ? 'bg-gradient-to-r from-gray-600 to-gray-700 text-white border-gray-600 shadow-lg transform scale-105' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400 hover:text-gray-700 hover:bg-gray-50' }}">
                                <div class="flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <!-- Frame Count Info -->
                    <div
                        class="flex items-center gap-3 bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-2.5 rounded-xl border border-gray-200">
            ...
            <div class="mt-12 frame-container">
                <div class="frame-loading-overlay" id="frame-loading-overlay">
                    <div class="bg-white rounded-lg p-6 flex items-center gap-3 shadow-lg">
                        <div class="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-[#BF3131]"></div>
                        <span class="text-gray-700">Memuat frame...</span>
                    </div>
                </div>
                <div id="frame-content">
                                        ...
                                        <div class="space-y-3">
                                            <div class="flex justify-between items-start">
                                                ...
                                                <div class="flex items-center gap-1.5 px-2 py-1 bg-red-50 rounded-full">
                                                    ...
                                                    <span
                                                        class="text-xs text-gray-700">{{ $frame->category->name }}</span>
                                                </div>
                                            </div>
                                                ...
                                                <div class="flex items-center justify-between">
                                                    <div
                                                        ...
                                                        <span class="text-sm font-semibold text-gray-800">
                                                            {{ number_format($frame->testimonis_avg_rating, 1) }}
                                                        </span>
                                                    </div>
                                                </div>
                                            @else
                    ...
                    @else
                        <div class="text-center py-20 bg-gradient-to-r from-red-50 to-red-50 rounded-2xl shadow-inner">
                            ...
                            <p class="text-xl text-gray-600 font-light">Belum ada frame yang tersedia untuk kategori ini.
                            </p>
    ...
    <script>
        ...
        function attachFilterListeners() {
            // Category links
            document.querySelectorAll('.category-link').forEach(link => {
                link.removeEventListener('click', handleFilterClick); // Remove existing listeners
                link.addEventListener('click', function(e) {
                    e.preventDefault();
                    handleFilterClick(this.href);
                });
            ...
            // Rating filter links
            document.querySelectorAll('.rating-filter-btn').forEach(link => {
                link.removeEventListener('click', handleFilterClick); // Remove existing listeners
                link.addEventListener('click', function(e) {
                    e.preventDefault();
                    handleFilterClick(this.href);
        ...
        function handleFilterClick(url) {
            ...
            console.log('Filter link clicked:', url);

            // Show loading overlay
            const loadingOverlay = document.getElementById('frame-loading-overlay');
            const frameContent = document.getElementById('frame-content');
            if (loadingOverlay && frameContent) {
                ...
                .then(html => {
                    ...
                    if (newContent) {
                        ...
                        if (currentContent) {
                            ...
                            setTimeout(() => {
                                const newLoadingOverlay = document.getElementById('frame-loading-overlay');
                                const newFrameContent = document.getElementById('frame-content');
                                if (newLoadingOverlay && newFrameContent) {
                                    newLoadingOverlay.classList.remove('show');
                                    newFrameContent.style.opacity = '1';
                                }
                                console.log('Content updated successfully');
                            }, 300); // Small delay for smooth transition
                        } else {
                            console.error('Current content_section not found');
                            toastr.error('Gagal memperbarui konten. Silakan coba lagi.', 'Error');
                            hideLoadingOverlay();
                        }
                    } else {
                        console.error('New content_section not found in response');
                        toastr.error('Gagal memuat filter. Silakan coba lagi.', 'Error');
                        hideLoadingOverlay();
                    }
                })
                .catch(error => {
                    console.error('Error fetching filtered content:', error);
                    toastr.error('Terjadi kesalahan saat memuat filter. Silakan coba lagi.', 'Error');
                    hideLoadingOverlay();
                });
        }
